...
export default async function Page({
    ...
    const data: AlbumPageResponse = await fetchAlbumSongs(
        id.toLowerCase().replace(" ", "-")
    );
    const credits: CreditsResponse = await fetchAlbumCredits(
        id.toLowerCase().replace(" ", "-")
    );
    ...
    return (
        <AlbumPageRootComponent
            data={data}
            //@ts-expect-error
            albumCreditData={credits.credits}
