...
function ActualAlbumPageStyle() {
    const [styleOption, setStyleOption] = useLocalStorage(
        "album-page-style",
        1
    );
    ...
    return (
        <div className="flex flex-col md:flex-row w-full h-full overflow-hidden">
            ...
            <div
                className={cn("md:w-[40%] h-full relative md:overflow-hidden")}
            ...
            >
                <div className="p-6 border-t-2 border-t-muted md:border-t-transparent md:border-l-2 md:border-l-muted h-fit md:h-screen rounded-t-2xl md:rounded-none bg-primary-foreground md:bg-primary-foreground/25 shadow-xl">
                    ...
                    <div className="flex flex-col gap-4 mt-6 [&>div]:overflow-hidden justify-center items-center grow">
                        <div
                            className={cn(
                                "rounded-xl p-4 flex justify-center items-center select-none cursor-pointer w-full h-full transition-all border-2 duration-300 bg-primary-foreground/50",
                                styleOption === 0 &&
                                    "bg-secondary border-primary"
                            )}
                            onClick={() => setStyleOption(0)}
                        >
                            <p>Normal style</p>
                        </div>
                        <div
                            className={cn(
                                "rounded-xl p-4 flex justify-center items-center select-none cursor-pointer w-full h-full transition-all border-2 gap-2 duration-300 bg-primary-foreground/50",
                                styleOption === 1 &&
                                    "bg-secondary border-primary"
                            )}
            ...
            <div className="md:-translate-x-[16px]">
                <div className="flex flex-col md:flex-row gap-4 transition-all duration-300">
                    <div className="flex-1">
                        <div
                            className={`flex gap-4 items-center p-4 md:p-8 mt-4 overflow-x-hidden pt-16 w-full justify-center md:justify-normal border-b-2 border-b-primary-foreground`}
                        >
                            <div className="flex flex-col md:flex-row items-center gap-5">
                                <Image
                                    src={`/song-files/covers/${id.toLowerCase()}.jpg`}
                                    alt={id}
                                    width={!useIsMobile() ? 260 : 280}
                                    height={!useIsMobile() ? 260 : 280}
                                    priority={true}
                                    className="md:mt-4 rounded-xl outline outline-primary/10"
                                />
                                <div className="flex flex-col gap-2">
                                    ...
                                    <div className="flex flex-col justify-center">
                                        <div className="inline-flex items-center justify-center md:justify-normal">
                                            <div className="text-md md:text-xl text-primary/75 whitespace-pre text-center md:text-left">
                                                {albumCreator || (
                                                    <Skeleton className="w-24 h-5" />
                                                )}
                                            </div>
                                            <Dot className="text-primary/75" />
                                            <div className="text-md md:text-xl text-primary/75 text-center md:text-left">
                                                {year || (
                                        ...
                                        <div className="inline-flex items-center justify-center md:justify-normal">
                                            <div className="text-md md:text-xl text-primary/75 whitespace-pre text-center md:text-left">
                                                {String("20") || (
                                                    <Skeleton className="w-5 h-5 translate-y-1 inline-flex" />
                                                )}{" "}
                                                songs
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex gap-2 justify-center md:justify-normal mt-2">
                                        <Button
                                            className={`rounded-full h-12 transition-all duration-300 justify-normal ${isPlaying ? "w-12" : "w-24"}`}
                                        >
                                            {!isPlaying ? <Play /> : <Pause />}
                                            <div
                                                className={`transition-all text-center ml-1 duration-300 ${isPlaying ? "opacity-0" : ""}`}
                                            >
                                                {!isPlaying
                                                    ? String("Play")
                                                    : String("")}
                        ...
                        <div className="p-2 bg-primary-foreground/25 mt-6 rounded-lg mx-4 md:mx-8 border-2 border-secondary/50 text-sm text-primary/50">
                        ...
                        <div className="m-4 md:m-8 md:mt-4 flex flex-col gap-4">
                            <div className="flex items-center relative">
                                <Input
                                    type="search"
                                    className="pl-[3em] border-2 border-secondary rounded-xl bg-primary-foreground/50"
                            ...
                            <div
                                className={cn(
                                    "transition-all duration-500 bg-primary-foreground/50 rounded-xl overflow-hidden border-2 border-secondary",
                                    appearBar ? "mb-24" : "-mb-4"
                                )}
                            >
                                {songs
                                    .filter((op: SongInterface) =>
                                        op.title
                                            .toLowerCase()
                                    ...
                                    .map((element, index) => (
                                        <div
                                            ...
                                            className={cn(
                                                ...
                                                currentSongIndex === index
                                                    ? "bg-primary/15 border-b-transparent"
                                        ...
                                        >
                                            ...
                                            <div className="select-none whitespace-pre overflow-hidden w-3/4 shadowed-song-name">
                                                <div className="text-sm font-semibold max-w-52">
                                                    {element.title || (
                                                        <Skeleton className="w-28 h-6 translate-y-0.5" />
                                                    )}
                                                </div>
...
function SecondStyleWIP() {
    return !useIsMobile() ? (
        <motion.div
            ...
            exit={{ opacity: 0, y: -40, filter: "blur(20px)" }}
            transition={{ duration: 0.6, ease: "easeInOut" }}
            className="p-6 pt-20 text-center m-auto h-fit w-fit"
        >
            This show-off is currently in WIP, so just test it yourself by going
            to an album page
        </motion.div>
    ) : (
        <div>
    ...
    return (
        ...
        >
            <div className="flex gap-4 flex-col">
                <div
                    className={
                        "flex gap-y-2 flex-col items-center justify-start"
                    }
                >
                    <div className="sticky top-5 flex flex-col gap-2">
                        <div
                            className={cn(
                                "flex flex-col gap-3 items-center justify-center rounded-xl relative p-4 px-8",
                                !useIsMobile()
                                    ? "border border-muted h-fit overflow-hidden"
                                    : "w-full h-full"
                            )}
                        >
                            <Image
                                src={`/song-files/covers/${id.toLowerCase()}.jpg`}
                                alt={`${id.toLowerCase()}`}
                                className="absolute inset-0 bg-cover bg-center opacity-10 blur-2xl size-full touch-none select-none z-10"
                                fill
                            />
                            <div className="flex gap-2 flex-col items-center justify-center">
                                <Image
                                    src={`/song-files/covers/${id.toLowerCase()}.jpg`}
                                    alt={id}
                                    width={!useIsMobile() ? 260 : 320}
                                    height={!useIsMobile() ? 260 : 320}
                                    priority={true}
