...
<Separator orientation="horizontal" className="h-[2px] rounded-full bg-muted/80" />
        ...
        is the way fans have to find the leaked material.
        \
        \
        If you want to know all the albums Ye has cancelled, you gotta go to the wiki page,
        which is horrible because the platform they use is filled with ads and is not user friendly at all.
        \
        \
        ...
        \
        \
        This website comes with the purpose of being a easy way for new and old fans to listen to the best versions without all the hasle that comes with it.
        The whole project is open source, so if you feel like there is a missing song/album, you can just create a issue report or (even better) crete a Pull Request with that thing!
<Separator orientation="horizontal" className="h-[2px] rounded-full bg-muted/80" />
        ## How
