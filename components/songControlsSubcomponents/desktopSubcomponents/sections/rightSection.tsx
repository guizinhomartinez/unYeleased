...
export default function RightSectionSongControls({
    image,
    songVal,
    id,
    songRef,
    ...
    const [lyricsOpened, setLyricsOpened] =
        useContext(LyricsOpened).lyricsOpened;
    ...
    return (
        <div
            className="flex items-center justify-end select-none gap-2 lg:w-full md:w-[25%]"
        ...
        >
            ...
            <div className="flex gap-2 items-center h-full">
                <VolumePopover
                    {...{ repeat, setVolumeVal, songRef, volumeVal }}
                />
                        ...
                        <Tooltip open={hoveredVolumeIcon}>
                            <TooltipTrigger asChild>
                                <Button
                                    onMouseLeave={() =>
                                        setTimeout(
                                            () => setHoveredVolumeIcon(false),
                                            150
                                        )
                                    }
                                    onMouseOver={() =>
                                        setTimeout(
                                            () => setHoveredVolumeIcon(true),
                                            250
                                        )
                                    }
            ...
            >
                <div className="relative rounded-lg w-full max-h-full overflow-hidden">
                    <Image
                        src={image}
                        alt={image}
                        className="absolute inset-0 bg-cover bg-center opacity-10 blur-2xl size-full"
                        fill
                    />
                    <div className="h-full p-2">
                        <Lyrics
                            currentTimeVal={Math.floor(
                                currentTimeVal * lyricsDelay
                            )}
                            id={id}
                            songVal={songVal}
                            isSynced={!isSynced}
