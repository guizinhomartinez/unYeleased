...
export const FullscreenButton = (props: FullscreenButtonInterface) => {
    ...
    useMotionValueEvent(x, "animationComplete", () => setIsDragging(false));

    const longPressEvent = useLongPress(
        () => {
            setSecondDrawerOpen(!isDragging ? true : false);
        },
        {
            onStart: () => {},
            threshold: 500,
        ...
        >
            ...
            >
                <Image
                    ...
                    className="absolute inset-0 bg-cover bg-center opacity-10 blur-2xl size-full pointer-events-none"
                    fill
                />
                <div
                ...
                >
                    <div className="flex justify-between items-center outline-none">
                        <motion.div
                            ...
                            style={{
                                maskImage:
                                    "linear-gradient(#000 5%, #000 95%, transparent 100%)",
                                WebkitMaskImage:
                                    "linear-gradient(#000 5%, #000 95%, transparent 100%)",
                                x,
                            }}
                        ...
                        >
                            <div className="flex justify-center items-center gap-3">
                                <Image
                                    ...
                                    width={60}
                                    height={60}
                                    className="rounded-lg shadow-xl pointer-events-none aspect-square"
                                />
                                <div className="flex flex-col w-full overflow-hidden">
                                    <AutoMarquee
                                        text={props.songVal || "No Track Found"}
                                        className="font-semibold max-w-48"
                                        number={0}
                                    />
                                    <AutoMarquee
                                        text={props.songCreator || "Unknown"}
                        ...
                        <div
                            className="flex gap-2 items-end justify-end min-w-9 overflow-hidden"
                            onClick={(e) => {
                                e.stopPropagation();
                                e.preventDefault();
                            }}
                ...
                <div className="relative size-full overflow-hidden px-4">
                    <Lyrics
                        currentTimeVal={props.currentTimeVal}
                        id={props.id}
                        songVal={props.songVal}
                        isSynced={!props.isSynced}
                        isFullscreenMode={false}
                        syncedLyricsClassName={clsx("pt-10")}
                        haveVerticalSpace={false}
                    />
                    <div className="absolute left-2.5 bottom-6">
                        <Button
                            size="icon"
                            variant="link"
                            className="bg-secondary/50 hover:bg-secondary p-1.5 rounded-full opacity-75 hover:opacity-100 z-[1000]"
                            onClick={() => setHidePill(!hidePill)}
                        >
                            <ChevronLeft
                                className={cn(
                                    "transition-all duration-500",
                                    !hidePill ? "rotate-0" : "rotate-180"
                                )}
                            />
                        </Button>
                    </div>
                    ...
                    >
                        <div className="rounded-full flex justify-center items-center w-full relative">
                            <div
                                onClick={() => props.setIsSynced(true)}
                                className={cn(
                                    "w-full text-center transition-colors duration-500 rounded-full cursor-default select-none",
                                    !props.isSynced && "cursor-pointer"
                                )}
                            >
                                Synced
                            </div>
                            <div
                                onClick={() => props.setIsSynced(false)}
                                className={cn(
                                    "w-full text-center transition-colors duration-500 rounded-full cursor-default select-none",
                                    props.isSynced && "cursor-pointer"
                                )}
