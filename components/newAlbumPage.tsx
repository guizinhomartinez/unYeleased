...
export default function NewAlbumPage({
    ...
    // should've never vibe coded this what does it even do???
    const FormatCreditsText = () => {
        return Object.entries(
            credits.reduce(
                (acc, credit) => {
                    const type = credit.type;
                    const names = Array.isArray(credit.name)
                        ? credit.name
                        : [credit.name];
                    const links = Array.isArray(credit.originalLink)
                        ? credit.originalLink
                        : [credit.originalLink];
    ...
    const CreditsDialogContent = () => {
        return Object.entries(
            ...
            >
                <Button
                    variant="link"
                    className="-mb-6 p-0 rounded-xl w-full justify-between"
                >
                    {entry.name}
                    <ArrowRightIcon
                        size="24"
    ...
    return (
        <>
            ...
            <div className="flex m-5 md:!pb-10 md:p-8 lg:!px-0 lg:!pt-0 gap-3 lg:mx-12 flex-col lg:flex-row max-w-full xl:max-w-7xl xl:mx-auto">
                <div className="flex gap-y-2 flex-col items-center justify-start w-full lg:w-96">
                    <div className="relative flex flex-col gap-2">
                        <div className="flex flex-col gap-3 items-center justify-center rounded-xl p-8 pb-4 w-full h-full lg:border lg:border-muted lg:h-fit lg:overflow-hidden lg:relative lg:w-[initial]">
                            <Image
                                src={props.albumCover}
                                alt={`${id.toLowerCase()}`}
                                className="absolute inset-0 bg-cover bg-center opacity-10 blur-2xl size-full touch-none select-none pointer-events-none"
                                fill
                            />
                            <div className="flex gap-2 flex-col items-center justify-center">
                                {props.albumCoverInfo.length === 0 ||
                                props.albumCoverInfo.length === 1 ? (
                                    <AlbumCover
                                        id={id}
                                        newAlbumPage={true}
                                    ...
                                    <AlbumCoverDialog
                                        ...
                                        albumCover={props.albumCover}
                                        albumName={albumName}
                                        albumCoverInfo={props.albumCoverInfo}
                                        albumCoverType={props.albumCoverType}
                                        setAlbumCoverType={
                                            props.setAlbumCoverType
                                        }
                                        albumCoverDescription={
                                            props.albumCoverDescription
                                        }
                                    />
                                )}
                        ...
                        <div
                            className={cn(
                                "rounded-xl h-fit border border-muted bg-primary-foreground/50 w-full max-w-full mx-auto flex flex-col gap-2 lg:mb-[10vh]",
                                credits !== undefined
                                    ? "p-4"
                ...
                <div className="my-2 h-px w-full bg-muted visible lg:hidden" />
