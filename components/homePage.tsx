...
const Albums = ({ entry, isGrid, setSearchQuery, index }: AlbumsInterface) => {
    ...
    if (!isMounted) {
        if (!isGrid) {
            return (
                <Skeleton className="flex gap-4 items-center p-3 rounded-xl border border-muted w-full shadow-md overflow-hidden" key={index} />
            );
        }
        return (
            <Skeleton className="w-full h-96 flex flex-col gap-3 rounded-2xl p-4 items-center border border-muted shadow-md" key={index} />
        );
    }
