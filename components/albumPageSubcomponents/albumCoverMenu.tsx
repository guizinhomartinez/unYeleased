...
export const AlbumCoverDialog = (props: {
    albumCover: string;
            ...
            <div className="flex flex-col md:flex-row items-center justify-center gap-4">
                <Carousel
                    opts={{
                        ...
                        loop: true,
                        startIndex:
                            props.albumCoverType - props.albumCoverInfo.length,
                    }}
                ...
                <div
                    className={cn(
                        "flex flex-col gap-2 max-w-full max-h-full mt-2 md:max-w-[50%] md:max-h-80 md:mt-1 min-[1700px]:!max-w-96"
                    )}
                >
                    <p className="text-2xl font-semibold leading-none tracking-tight text-center md:text-left">
                        {props.albumName}'s alternative covers
                    </p>
                    <p
                        className="text-md text-muted-foreground text-center md:text-left"
                        ref={textRef}
                    >
                        Check these other album covers that were made for this
                        album but were scrapped.
                    </p>
                    <div
                        className="w-full h-0.5 bg-secondary rounded-full"
                        style={{
                            display:
                                props.albumCoverDescription.length === 0 ||
                                props.albumCoverDescription === undefined
                                    ? "block"
                                    : "none",
                        }}
                    />
    ...
    const SelectButtons = (props: {
        ...
        const ApplyButton = () => {
            return (
                <Button
                    className={cn(
                        "rounded-full py-5 w-full",
                        isMobile ? "w-full" : "w-[60%]"
                    )}
                    onClick={() => {
                        toast.success("Applied album cover!");

                        props.setAlbumCoverType(
                            emblaRef.current?.selectedScrollSnap()
                        );
    ...
    return (
        <>
            <div className="group relative size-fit overflow-hidden">
                ...
                <AlbumCover
                    id={props.id}
                    newAlbumPage={props.newAlbumPage}
            ...
            {isMobile ? (
                ...
                >
                    <DrawerContent
                        className="h-full max-h-full rounded-t-none"
                        showGrabThing={false}
                    >
                        ...
                        <Button
                            className="min-w-9 absolute right-4 top-4 opacity-70 ring-offset-background transition-opacity hover:opacity-100 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground bg-secondary/50 hover:bg-secondary p-1.5 rounded-full"
                            onClick={() => setDialogOpened(false)}
                        >
                            <X className="min-h-4 min-w-4 rounded-full text-primary" />
                        </Button>
                        <DrawerFooter>
                            <div className="absolute bottom-0 left-0 w-full h-36 bg-gradient-to-b from-transparent to-primary-foreground to-95%" />
                            <div className="flex gap-4 items-center justify-center">
                                <SelectButtons
                                    isMobile={false}
                                    setAlbumCoverType={props.setAlbumCoverType}
                                />
                            </div>
                        </DrawerFooter>
                    </DrawerContent>
                ...
                <Dialog open={dialogOpened} onOpenChange={setDialogOpened}>
                    ...
                    <DialogContent
                        className={cn(
                            "lg:max-w-[70%] md:max-w-[80%] !rounded-3xl p-6",
                            reallyBigScreen && "!max-w-[56rem]"
                        )}
                        dialogCloseButton={false}
                    >
                        <Button
                            className="min-w-9 absolute right-4 top-4 opacity-70 ring-offset-background transition-opacity hover:opacity-100 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground bg-secondary/50 hover:bg-secondary p-1 rounded-full"
                            onClick={() => setDialogOpened(false)}
                        >
                            <X className="min-h-4 min-w-4 rounded-full text-primary" />
                        </Button>
